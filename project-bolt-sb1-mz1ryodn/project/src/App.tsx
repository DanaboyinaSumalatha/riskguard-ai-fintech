    ...
    <div className="min-h-screen bg-[#050A14] text-white relative overflow-hidden">
      ...
      <header className="relative z-20 max-w-7xl mx-auto mt-4 mx-4 md:mx-auto">
        <div className="flex justify-between items-center px-6 py-3 rounded-full bg-[#0F1C2E]/60 backdrop-blur-xl border border-white/[0.08]">
          <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-xl bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center"><Shield size={20}/></div><span className="text-[18px] font-bold">RiskGuard AI</span><div className="hidden md:flex items-center gap-6 ml-10 text-[13px] text-white/40"><span>Product</span><span>Features</span><span>Pricing</span><span>Security</span><span>Docs</span></div></div>
          <div className="flex gap-3 items-center">
            {!showDashboard? (<><button onClick={()=>setShowLogin(true)} className="px-5 py-2 rounded-full bg-white/[0.06] border border-white/10 text-[13px]">Login</button><button onClick={()=>setShowLogin(true)} className="px-6 py-2.5 rounded-full bg-[#5EE1FF] text-black font-bold text-[13px]">Get Started</button></>):(<div className="flex items-center gap-2"><span className="text-[11px] flex gap-2 items-center bg-white/[0.06] border border-white/10 px-3 py-1.5 rounded-full"><User size={12}/>{email}</span><button onClick={()=>{setShowDashboard(false); setEmail('');}} className="px-4 py-1.5 rounded-full bg-white/10 text-[11px] flex gap-1 items-center"><LogOut size={12}/>Logout</button></div>)}
          </div>
        </div>
      </header>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 md:px-16 pt-12 pb-8 text-center">
        ...
        {/* ✅ DIRECT SVG - NO IMG - 100% WORK */}
        <div className="mt-12 relative mx-auto w-[400px] md:w-[560px] flex flex-col items-center">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-cyan-400/20 blur-[90px] rounded-full"></div>

          <svg viewBox="0 0 400 480" className="relative w-[380px] md:w-[460px] h-auto drop-shadow-[0_0_80px_rgba(94,225,255,0.8)]">
            <defs>
              <radialGradient id="g1" cx="50%" cy="45%" r="60%"><stop offset="0%" stopColor="#5EE1FF" stopOpacity="0.8"/><stop offset="100%" stopColor="#0A1930" stopOpacity="0"/></radialGradient>
              <linearGradient id="g2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#7FE8FF" stopOpacity="0.9"/><stop offset="50%" stopColor="#2DD4FF" stopOpacity="0.4"/><stop offset="100%" stopColor="#0F2A5A" stopOpacity="0.8"/></linearGradient>
            </defs>
            <ellipse cx="200" cy="240" rx="190" ry="230" fill="url(#g1)" opacity="0.6"/>
            <path d="M200 20 L360 80 L360 200 Q360 360 200 460 Q40 360 40 200 L40 80 Z" fill="url(#g2)" stroke="#A5F3FF" strokeWidth="3" strokeOpacity="0.8"/>
            <path d="M200 35 L340 90 L340 200 Q340 340 200 435 Q60 340 60 200 L60 90 Z" fill="none" stroke="white" strokeWidth="1.5" strokeOpacity="0.3"/>
            <ellipse cx="140" cy="110" rx="70" ry="35" fill="white" opacity="0.25" transform="rotate(-20 140 110)"/>
            <g stroke="#5EE1FF" strokeWidth="1.2" opacity="0.7" fill="none">
            ...
            <circle cx="200" cy="230" r="35" fill="none" stroke="#5EE1FF" strokeWidth="1" opacity="0.4"/>
            <circle cx="200" cy="230" r="22" fill="none" stroke="#A5F3FF" strokeWidth="1.5" opacity="0.6"/>
            <circle cx="200" cy="230" r="12" fill="#5EE1FF" opacity="0.9"/>
            <circle cx="200" cy="230" r="6" fill="white"/>
            <g><rect x="135" y="290" width="130" height="22" rx="11" fill="#0B2A4A" stroke="#5EE1FF" strokeOpacity="0.5"/><text x="200" y="305" textAnchor="middle" fontFamily="Arial" fontSize="10" fill="#7FE8FF" letterSpacing="1">AI PROTECTED</text></g>
          </svg>
        ...
        <div className="mt-10 grid md:grid-cols-3 gap-4 max-w-4xl mx-auto">
          ...
          <div className="bg-[#0F1C2E]/60 border border-white/[0.08] rounded-2xl p-5"><p className="font-bold">Sub-50ms Detection</p><p className="text-[11px] text-white/40">Real-time response</p></div>
      ...
      {showDemo && (<div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4"><div className="bg-[#0F172A] border-2 border-white/[0.12] rounded-[28px] p-8 w-full max-w-5xl max-h-[90vh] overflow-y-auto"><button onClick={()=>setShowDemo(false)} className="absolute top-5 right-5 w-9 h-9 rounded-full bg-white/10 flex items-center justify-center"><X size={16}/></button><h3 className="text-[24px] font-bold">Transaction Fraud Checker</h3><div className="grid md:grid-cols-3 gap-5 mt-8"><div className="bg-[#060A18] border border-white/10 rounded-2xl p-4"><input value={amount} onChange={e=>setAmount(e.target.value)} className="w-full bg-[#10182E] border border-white/10 rounded-xl p-3 text-sm mb-2"/><select value={country} onChange={e=>setCountry(e.target.value)} className="w-full bg-[#10182E] border border-white/10 rounded-xl p-3 text-xs mb-2"><option>Same country</option><option>High-risk country</option></select><select value={time} onChange={e=>setTime(e.target.value)} className="w-full bg-[#10182E] border border-white/10 rounded-xl p-3 text-xs"><option>Normal hours</option><option>3 AM (Suspicious)</option></select></div><div className="bg-[#060A18] border border-white/10 rounded-2xl p-4"><div className="bg-[#10182E] rounded-xl p-4 text-[11px]">${amount} • {country}</div><button onClick={runCheck} className="w-full py-3.5 bg-white text-black rounded-full font-bold text-sm mt-4">Analyze →</button></div><div className="bg-[#060A18] border border-white/10 rounded-2xl p-4">{!result?<div className="text-xs text-white/30">Click Analyze</div>:<div><div className="text-[36px] font-bold">{result.split('|')[1]}/100</div><div className="font-bold">{result.split('|')[0]}</div></div>}</div></div></div></div>)}
    </div>
  );
}
