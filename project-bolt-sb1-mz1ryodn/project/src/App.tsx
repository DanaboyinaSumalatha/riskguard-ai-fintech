...
import { Shield, CreditCard, Brain, LogOut, User, X } from 'lucide-react';

export default function App() {
  const [showLogin, setShowLogin] = useState(false);
  const [showDashboard, setShowDashboard] = useState(false);
  const [showDemo, setShowDemo] = useState(false); // MODAL KOSAM IDHI ADD CHESA
    ...
    <div className="min-h-screen bg-[#050A14] text-white">
      ...
      {!showDashboard? (
        <>
          <section className="max-w-7xl mx-auto px-6 pt-10 pb-10 grid md:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <div className="mt-6 flex gap-3">
                ...
                <button onClick={()=>setShowDemo(true)} className="px-6 py-3 rounded-full bg-cyan-500 text-black font-bold text-sm">🚀 Try Live Demo</button>
              </div>
            </div>
          ...
          {/* NORMAL GA PAGE LO CARD LEKUNDA TESESA - MODAL LO MATRAM VASTUNDI */}
          <section className="max-w-5xl mx-auto p-6 text-center">
            <button onClick={()=>setShowDemo(true)} className="px-8 py-4 rounded-full bg-[#0F172A] border border-cyan-500/30 text-cyan-300 font-bold shadow-[0_0_30px_rgba(6,182,212,0.2)] hover:bg-[#10182E] transition">
